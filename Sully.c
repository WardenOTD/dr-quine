...
int main(){
	...
	char* str = "#include <stdio.h>%c#include <fcntl.h>%c#include <unistd.h>%c#include <string.h>%c%c#define FILENUM (__FILE__[strlen(__FILE__) - 3] == 'y') ? 4 : (__FILE__[strlen(__FILE__) - 3] - 49)%c%cint main(){%c%cint i = %d;%c%cif (--i < 0)%c%c%creturn 0;%c%cint len = snprintf(NULL, 0, %cSully_%cd.c%c, i);%c%cchar filename[len + 1];%c%csnprintf(filename, len + 1, %cSully_%cd.c%c, i);%c%c";
